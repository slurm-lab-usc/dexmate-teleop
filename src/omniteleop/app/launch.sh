...
# Plus the recorder subprocesses (mcap_recorder, mdp_recorder) and the replay
# publisher (replay_record), which run as `python -m omniteleop.record.<module>`
# so we match the module path too.
